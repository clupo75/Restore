...
export default function OrderSummary() {
    // fetch the basket from the store using RTK Query
    const { data: basket } = useFetchBasketQuery();
    // calculate the subtotal and delivery fee using the basket data and reduce method
    const subtotal = basket?.items.reduce((sum: number, item: Item) => sum + (item.price * item.quantity), 0) ?? 0;
    // free delivery for orders over $100, otherwise $5 delivery fee
